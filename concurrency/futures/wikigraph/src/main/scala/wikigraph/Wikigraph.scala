...
final class Wikigraph(client: Wikipedia):
  ...
  def namedLinks(of: ArticleId): WikiResult[Set[String]] =
    client.linksFrom(of).flatMap { articles =>
      WikiResult.traverse(articles.toSeq)(client.nameOfArticle)
    }.map(_.toSet)
  ...
  def breadthFirstSearch(start: ArticleId, target: ArticleId, maxDepth: Int): WikiResult[Option[Int]] =
    ...
    def iter(visited: Set[ArticleId], q: Queue[(Int, ArticleId)]): WikiResult[Option[Int]] =
      if q.isEmpty then
        ...
        val (depth, article) = q.head
        if depth >= maxDepth then WikiResult.successful(None)
        else
          if article == target then
            WikiResult.successful(Some(depth))
          else
            client.linksFrom(article).flatMap { ids =>
              val idsWithDepth = ids.filter(!visited.contains(_)).map((depth + 1, _))
              iter(visited + article, q.dequeue._2.enqueueAll(idsWithDepth))
            }
            .fallbackTo{
              iter(visited + article, q.dequeue._2)
            }
    end iter
  ...
  def distanceMatrix(titles: List[String], maxDepth: Int = 50): WikiResult[Seq[(String, String, Option[Int])]] =
    val pairs = for
      title1 <- titles
      title2 <- titles.filter(_ != title1)
    yield (title1, title2)
    WikiResult.traverse(pairs) { (titleFrom, titleTo) =>
      for
        idFrom   <- client.searchId(titleFrom)
        idTo     <- client.searchId(titleTo)
        distance <- breadthFirstSearch(idFrom, idTo, maxDepth)
        result   <- WikiResult.successful((titleFrom, titleTo, distance))
      yield result
    }
end Wikigraph
