...
object Firework:
  ...
  def next(firework: Firework): Firework =
    firework match
      case fw: Waiting   => fw.next
      case fw: Launched  => fw.next
      case fw: Exploding => fw.next
      case Done          => Done
...
case class Waiting(countDown: Int, startPosition: Point, numberOfParticles: Int, particlesColor: Color) extends Firework:
  ...
  def next: Firework =
    if countDown > 0 then
      this.copy(countDown = countDown - 1)
    else 
...
case class Launched(countDown: Int, position: Point, direction: Angle, numberOfParticles: Int, particlesColor: Color) extends Firework:
  ...
  def next: Firework =
    if countDown > 0 then
      this.copy(countDown - 1, Motion.movePoint(position, direction, Settings.propulsionSpeed), direction, numberOfParticles, particlesColor)
    else
...
case class Exploding(countDown: Int, particles: Particles) extends Firework:
  ...
  def next: Firework =
    if countDown > 0 then
      this.copy(countDown - 1, particles.next)
    else Done
